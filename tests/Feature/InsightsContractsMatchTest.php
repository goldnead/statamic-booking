 ...
 * `require-dev`, so those `interface_exists` locks never engage: the copies are
 * what the whole suite runs against **and** what PHPStan analyses through
...
class InsightsContractsMatchTest extends TestCase
{
     ...
     * every duration into a plain count.
     */
     ...
     *
     * `TableMetric` is not a contract, it is **inherited behaviour**: the
     * windowing, the three SQL dialects and the null-preserving split all run
     * from its body, and every metric in `src/Integrations/Insights` gets its
     * numbers out of them. A signature comparison would pass over a fixed
     * `bucketExpression`, a changed `orderByRaw` or a null row that upstream
     * stopped dropping — the suite would stay green while proving the arithmetic
     * of a class no installation runs. Most of what matters is `protected`
     * anyway and never appears in a public shape at all.
     *
     * So the stand-in is not a rewrite but a copy, and this asserts exactly
     * that. When it fails, the fix is `cp`, then run the metric tests: they are
     * the ones that say whether the new behaviour still adds up.
     */
    #[Test]
    public function the_copied_base_class_is_the_real_file(): void
    {
        ...
        $this->assertFileEquals(
            $quelle.'/Support/TableMetric.php',
            dirname(__DIR__).'/Fakes/insights-table-metric.php',
            'tests/Fakes/insights-table-metric.php is meant to be the sibling’s TableMetric verbatim, and it is not any more. '
    ...
    protected function keinGeschwister(): string
    {
        return 'goldnead/statamic-insights was not found — neither in vendor/ nor checked out beside this package. '
            ...
            .'under tests/Fakes still match the real contract. Run this where the sibling exists.';
    }
